...
trait JSMaps extends Base {
  ...
  def map_unsafe_update[K:Manifest,V:Manifest](m: Rep[Map[K,V]], k: Rep[K], v: Rep[V]): Rep[Unit]
  ...
  def map_values[K:Manifest,V:Manifest](m: Rep[Map[K,V]]): Rep[Iterable[V]]
  def map_clear[K:Manifest,V:Manifest](m: Rep[Map[K,V]]): Rep[Unit]
  def map_keys[K:Manifest,V:Manifest](m: Rep[Map[K,V]]): Rep[Iterable[K]]
...
trait JSMapsExp extends JSMaps with EffectExp with TupleOpsExp {
  ...
  case class MapValues[K:Manifest,V:Manifest](m: Exp[Map[K,V]]) extends Def[Iterable[V]]
  case class MapClear[K:Manifest,V:Manifest](m: Exp[Map[K,V]]) extends Def[Unit]
  case class MapKeys[K:Manifest,V:Manifest](m: Exp[Map[K,V]]) extends Def[Iterable[K]]
  ...
  def map_update[K:Manifest,V:Manifest](m: Exp[Map[K,V]], k: Exp[K], v: Exp[V]) = reflectWrite(m)(MapUpdate(m,k,v))
  def map_unsafe_update[K:Manifest,V:Manifest](m: Exp[Map[K,V]], k: Exp[K], v: Exp[V]) = reflectEffect(MapUpdate(m,k,v))
  ...
  def map_clear[K:Manifest,V:Manifest](m: Exp[Map[K,V]]) = reflectWrite(m)(MapClear(m))
...
trait JSGenMaps extends JSGenEffect with QuoteGen {
  ...
  override def emitNode(sym: Sym[Any], rhs: Def[Any]) = rhs match {
    ...
    case MapClear(m)            => emitValDef(sym,
      q"""Object.keys($m).forEach(function(el,_i,_a) {
    ...
    case MapMapValues(m, v, b)  =>
      ...
      stream.println(q"$v = $m[_k];")
      emitBlock(b)
      stream.println(q"$sym[_k] = ${getBlockResult(b)}")
      stream.println("})")
    case MapFilter(m, k, v, b)  =>
      stream.println(q"var $sym = {};")
      stream.println(q"Object.keys($m).forEach(")
      stream.println(q"function($k,_i,_a){")
      stream.println(q"$v = $m[$k];")
      emitBlock(b)
      stream.println(q"if(${getBlockResult(b)}) {")
