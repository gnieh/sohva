...
class TestJsonDiff extends FlatSpec with ShouldMatchers {
  ...
  "a diff" should "be empty if created between two equal values" in {
    val json = parse("true")
    diff(json, json) should be(Patch(Nil))
  }

  it should "be a simple replacement if the two values are completely different" in {
    diff(parse("true"), parse("13")) should be(Patch(Replace(Nil, JInt(13))))
  }
  ...
  it should "contain an add operation for each added field" in {
    ...
    diff(json1, json2) should be(Patch(Add("new", JBool(false))))
    diff(json1, json3) should be(Patch(Add("new1", JBool(false)), Add("new2", JNull)))
    diff(json4, json5) should be(Patch(Add("b" :: "b", JInt(43)), Add("c", JNull)))
  }
  ...
  it should "contain a remove operation for each removed field" in {
    ...
    val json4 = parse("""{"a": 3, "b": {"a": true }}""")
    val json5 = parse("""{"a": 3, "b": {"a": true, "b": 43}, "c": null}""")
    diff(json2, json1) should be(Patch(Remove("old")))
    diff(json3, json1) should be(Patch(Remove("old1"), Remove("old2")))
    diff(json5, json4) should be(Patch(Remove("b" :: "b"), Remove("c")))
  }
  ...
  it should "contain a replace operation for each changed field value" in {
    ...
    diff(json1, json2) should be(Patch(Replace("lbl", JInt(60))))
    diff(json1, json3) should be(Patch(Replace("lbl", parse("""{"a": true}"""))))
    diff(json3, json4) should be(Patch(Replace("lbl" :: "a", JNull)))
  }
